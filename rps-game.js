class RpsGame {
  ...
  _regiserTurnResult(turn, playerIndex) {
    ...
    if (firstFreeX == -1) {
      // c'est plein il aurait pas du pouvoir cliquer la, encore a lui de jouer
    } else {
  ...
  _checkGameOver(cell, playerIndex) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (
        this._state[cell[0]][parseInt(cell[1]) + parseInt(i)] != undefined &&
      ...
      // puis horizontal
      if (
        hAddContinue && // si on a pas atteint une case d'une autre couleur
        this._state[parseInt(cell[0]) + parseInt(i)] != undefined && // que la pos en y existe
        this._state[parseInt(cell[0]) + parseInt(i)][parseInt(cell[1])] ==
          playerIndex
      ) {
        // que la couleur correspond
        horizontal++;
      } else hAddContinue = false;
      if (
        hMinusContinue &&
        this._state[parseInt(cell[0]) - parseInt(i) - 1] != undefined &&
        this._state[parseInt(cell[0]) - parseInt(i) - 1][parseInt(cell[1])] ==
          playerIndex
      ) {
        horizontal++;
      ...
      if (
        d1AddContinue && // si on a pas atteint une case d'une autre couleur
        this._state[parseInt(cell[0]) + parseInt(i)] != undefined && // que la pos en y existe
        this._state[parseInt(cell[0]) + parseInt(i)][
          parseInt(cell[1]) + parseInt(i)
        ] == playerIndex
      ) {
        // que la couleur correspond
        diag1++;
      ...
      if (
        d1MinusContinue &&
        this._state[parseInt(cell[0]) - parseInt(i) - 1] != undefined &&
        this._state[parseInt(cell[0]) - parseInt(i) - 1][
          parseInt(cell[1]) - parseInt(i) - 1
        ] == playerIndex
      ) {
        diag1++;
      ...
      if (
        d2AddContinue && // si on a pas atteint une case d'une autre couleur
        this._state[parseInt(cell[0]) + parseInt(i)] != undefined && // que la pos en y existe
        this._state[parseInt(cell[0]) + parseInt(i)][
          parseInt(cell[1]) - parseInt(i)
        ] == playerIndex
      ...
      if (vertical >= 4 || horizontal >= 4 || diag1 >= 4 || diag2 >= 4) {
        return true;
      }
    }
    return false;
  ...
  _sendScore() {
    this._players.forEach((player) =>
      player.emit("score", this._score.join(" - ")),
    );
  }
}
